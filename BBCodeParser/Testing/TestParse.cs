...
namespace Testing
{
    ...
    public class TestParse
    {
            ...
            bbCodeParser = new BbParser(new[]
                                         {
											 ...
											 new Tag("quote", $"<div class=\"{QuoteClassName}\"><div class=\"{QuoteAuthorClassName}\">{{value}}</div>", "</div>", true),
											 new Tag("tab", "&nbsp;&nbsp;&nbsp;"),
	    ...
	    public void TestOptionalAttribute()
	    {
			...
			Assert.AreEqual("<div class=\"quote\"><div class=\"quote-author\">Author</div>test</div>", actual1.ToHtml());
		...
		{
			var input = "[b]Not inside the code[/b]... [i]not yet[/i]. [code]And [b]this one is[/b]<script></script>[/code]";
			var actual = bbCodeParser.Parse(input);
			Assert.AreEqual("<strong>Not inside the code</strong>... <em>not yet</em>. <pre class=\"code\">And [b]this one is[/b]&lt;script&gt;&lt;/script&gt;</pre>", actual.ToHtml());
			Assert.AreEqual(input, actual.ToBb());
		}
		...
		{
			var input = "[link=\"javascriptJa vA s\"'\"'    \"'''\"\"\"\"   cript::alert(\"/Preved, admincheg/\")\"]Preved[/link]";
			var actual = bbCodeParser.Parse(input);
...
test [s]test[/s]";
        ...
        public void TestFilter()
        {
            ...
            Func<Node, bool> filter = n => !(n is TagNode) || ((TagNode) n).AttributeValue.Split(',').Select(v => v.Trim()).Any(v => v.Equals("Test3", StringComparison.InvariantCultureIgnoreCase));
        ...
        public void TestAliasSubstitutions()
        {
            ...
            Assert.AreEqual("Hello&lt; &ndash;<pre class=\"pref\"> <strong>&amp;world ---&gt;</strong></pre>", actual2.ToHtml());
